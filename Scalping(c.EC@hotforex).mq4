...
int init() {
	...
	magic = GetExecuteId();
	long_tp_size = StandardPointSize() *  4;
	long_sl_size =  StandardPointSize() *  30;
	short_tp_size = StandardPointSize() * 4;
	short_sl_size = StandardPointSize() * 30;
}
...
double getLongTP(int minutes) {
	int n = minutes/30;
	double ret =  long_tp_size - n*StandardPointSize()*1.5;
	return (ret);
}

double getShortTP(int minutes) {
	int n = minutes/30;
	double ret = short_tp_size - n*StandardPointSize()*1.5;
	return(ret);
}
