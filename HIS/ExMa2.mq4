...
void CreatePosition(int cmd){
  ...
  if (cmd==OP_BUY)
    OrderSend(Symbol(),OP_BUY,getLots(),Ask,3,Ask-stoploss,Ask+takeprofit,"",MAGICMA,0,Blue);    
  if (cmd==OP_SELL)
    OrderSend(Symbol(),OP_SELL,getLots(),Bid,3,Bid+stoploss,Bid-takeprofit,"",MAGICMA,0,Red); 
}
...
{  
  ...
  if( ma2<high0 && ma1>high0 ){
    ...
    //if (dayMa10>dayMa20)    
    if(PositionCountByCmd(OP_BUY)==0)
  ...
  if( ma2>low0 && ma1<low0 ){
    ...
    if(PositionCountByCmd(OP_SELL)==0)
